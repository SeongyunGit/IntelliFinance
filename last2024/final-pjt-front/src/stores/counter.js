...
export const useCounterStore = defineStore('counter', () => {
  ...
  // 공지사항 생성
const createAnnouncement = function (announcementData) {
  axios({
    method: 'post',
    url: `${API_URL}/accounts/announcement/create/`,
    data: announcementData,
    headers: {
      Authorization: `Token ${token.value}`  // 관리자 인증을 위한 토큰
    }
  })
    .then((response) => {
...
// 댓글 수정
const commentsUpdate = function (comment_pk, data) {
  axios({
    method: 'put',
    url: `${API_URL}/api/v1/comments/${comment_pk}/update/`,
    data: data,
    headers: {
      Authorization: `Token ${token.value}`
    }
  ...
  return { companyList, companyListOption, 
    ...
    getexchange, exchangelist, isCommentVisible, indexnumber, getexchange2,
    createAnnouncement, commentsUpdate
   }
}, { persist: true })
