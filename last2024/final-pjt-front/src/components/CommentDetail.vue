      ...
      <div class="relative p-6">
        ...
        <template v-else>
          <div class="space-y-4">
            ...
            <div class="flex items-center gap-2 px-3 py-1.5 bg-gray-50 rounded-lg">
              ...
              <div class="flex">
                <button
                  ...
                  :class="{
                    'text-yellow-400': tempRating >= n,
                    'text-gray-200': tempRating < n,
                  }"
                >
                  ★
                </button>
            ...
            <textarea
              v-model="tempComment"
              class="w-full h-32 p-4 bg-gray-50 border border-gray-200 rounded-xl resize-none focus:ring-2 focus:ring-blue-300 focus:border-transparent transition-all duration-200"
            ></textarea>
            ...
            <div class="flex justify-end gap-2">
              <button
                @click="cancelEdit"
                class="px-3 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-100 rounded-lg transition-colors duration-200"
              >
                취소
              </button>
              <button
                @click="updateComment(comment.id)"
                class="px-3 py-1.5 text-sm font-medium text-white bg-blue-500 hover:bg-blue-600 rounded-lg transition-colors duration-200"
              >
                저장
              </button>
            </div>
        ...
        <!-- 버튼 그룹 -->
        <div v-if="comment.user === store.mPK" class="absolute top-4 right-4 flex gap-2">
          <button
            v-if="editingCommentId !== comment.id"
            @click="startEdit(comment)"
            class="px-3 py-1.5 text-sm font-medium text-blue-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors duration-200"
          >
            수정
          </button>
          <button
            @click="store.commentsDelete(comment.id)"
            class="px-3 py-1.5 text-sm font-medium text-red-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors duration-200"
          >
            삭제
          </button>
        </div>
...
const startEdit = (comment) => {
  ...
  tempComment.value = comment.comment
  tempRating.value = parseInt(comment.star)
}
...
const updateComment = (commentId) => {
  const data = {
    ...
    star: tempRating.value.toString()
  }

  store.commentsUpdate(commentId, data)
  editingCommentId.value = null
}
</script>
