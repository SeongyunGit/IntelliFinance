<template>
  <div>
    <h3>mortgageLoan List</h3>
    <p>{{ store.surveyData.mortgageLoan }}</p>
    <div v-for="bank in store.integrationProducts"
      :key="bank.id"
      :bank="bank">
      <MortgageLoanListItem
        v-if="bank.type_a === 'mortgageLoan' 
          && store.surveyData.mortgageLoan.kor_co_nm.includes(bank.kor_co_nm)
          && store.integrationProductOptions.some(option => 
            option.fin_prdt_cd === bank.fin_prdt_cd
            && store.surveyData.mortgageLoan.lend_rate_type_nm.includes(option.lend_rate_type_nm)
            && store.surveyData.mortgageLoan.lend_rate.includes(option.lend_rate)
            ...
            && store.surveyData.mortgageLoan.mrtg_type_nm.includes(option.mrtg_type_nm)
          )"
        :bank="bank"
      />
    </div>
