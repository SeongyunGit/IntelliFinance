<template>
  <div>
    <h3>saving List</h3>
    <p>{{ store.surveyData.saving }}</p>
    <div v-for="bank in store.integrationProducts"
      :key="bank.id"
      :bank="bank">
      <SavingListItem
        v-if="bank.type_a === 'saving' && store.surveyData.saving.kor_co_nm.includes(bank.kor_co_nm) 
               && store.integrationProductOptions.some(option => 
                  option.fin_prdt_cd === bank.fin_prdt_cd
                  && store.surveyData.saving.intr_rate_type_nm.includes(option.intr_rate_type_nm)
                  && store.surveyData.saving.save_trm.includes(option.save_trm)
                  ...
                  && store.surveyData.saving.rsrv_type_nm.includes(option.rsrv_type_nm))"
        :bank="bank"
      />
    </div>
  </div>
