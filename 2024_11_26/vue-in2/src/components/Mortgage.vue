<template>
  <div class="container mx-auto min-h-screen bg-gradient-to-b from-indigo-600 to-indigo-500 p-8">
    <h3 class="text-3xl font-bold text-center text-white mb-8">전세 자금 대출</h3>
    <!-- 카드 리스트 -->
    <div class="grid grid-cols-3 gap-4 p-4">
      <template v-for="(bank, index) in store.integrationProducts.filter(product => product.type_a === 'mortgageLoan')" :key="bank.id">
        <MortgageItem
...
import MortgageItem from "./MortgageItem.vue";
...
const shouldShowComment = (currentIndex) => {
  ...
  // 전체 상품 목록의 길이를 가져옴
  const totalItems = store.integrationProducts.filter(product => product.type_a === 'mortgageLoan').length;
