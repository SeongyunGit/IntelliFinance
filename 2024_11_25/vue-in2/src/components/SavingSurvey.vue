        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 space-y-4">
          ...
          <div class="grid grid-cols-2 gap-3">
            <label 
              ...
              :key="type"
              class="flex flex-col items-center justify-center p-4 rounded-lg cursor-pointer transition text-center border border-purple-300"
              :class="surveyData.rsrv_type_nm.includes(type) 
                ? 'bg-indigo-50 border-2 border-indigo-500' 
                : 'hover:bg-gray-50'"
            >
              <input 
...
const allBanks = ['우리은행', '한국스탠다드차타드은행', '아이엠뱅크', '부산은행', '광주은행', '제주은행', '전북은행', '경남은행', '중소기업은행', '한국산업은행', '국민은행','농협은행주식회사', '하나은행', '주식회사 케이뱅크', '수협은행', '주식회사 카카오뱅크', '토스뱅크 주식회사'];
