...
const submitSurvey = () => {
  // 데이터를 저장하기 위한 객체
  const newSurveyData = {
    'kor_co_nm': surveyData.value['kor_co_nm'],
    'mtrt_int': surveyData.value['mtrt_int'],
    'intr_rate_type_nm': surveyData.value['intr_rate_type_nm'],  // 단일 값으로 수정
    'save_trm': surveyData.value['save_trm'],  // 단일 값으로 수정
    'intr_rate': surveyData.value['intr_rate'],
    'intr_rate2': surveyData.value['intr_rate2']
  }
  // 데이터가 이미 존재하는지 확인
  if (store.ox) {
    // 데이터가 이미 존재하면 업데이트
